...
class LocalTerminalProvider(
    ...
    private suspend fun createHiddenExecShell(executorKey: String): HiddenExecShell {
        var lastReadyResult: HiddenExecResult? = null

        repeat(HIDDEN_EXEC_START_MAX_ATTEMPTS) { attemptIndex ->
            val process =
                withContext(Dispatchers.IO) {
                    buildProcessBuilder(
            ...
            Log.w(
                TAG,
                "Hidden exec shell start attempt ${attemptIndex + 1}/$HIDDEN_EXEC_START_MAX_ATTEMPTS failed for $executorKey: ${readyResult.error}; preview=${readyResult.rawOutputPreview.takeLast(240)}"
            )
            destroyHiddenExecShell(shell)
            if (attemptIndex < HIDDEN_EXEC_START_MAX_ATTEMPTS - 1) {
                delay(500L)
            }
        }

        val fallbackMessage = lastReadyResult?.error ?: "Hidden exec shell did not become ready"
        val preview = lastReadyResult?.rawOutputPreview?.takeLast(600).orEmpty()
        throw IllegalStateException(
            if (preview.isBlank()) {
                fallbackMessage
            } else {
                "$fallbackMessage | startup output: $preview"
            }
        )
    }
    ...
    private suspend fun awaitHiddenExecReady(shell: HiddenExecShell): HiddenExecResult {
        return try {
            val rawOutput: String =
                withTimeout(HIDDEN_EXEC_READY_TIMEOUT_MS) {
    ...
    private fun extractVisibleOutput(
        rawOutput: String,
        token: String
    ): String {
        ...
        var visibleOutput = rawOutput.substring(payloadStart).trimStart('\n', '\r')
        val endMarkerPrefix = "$END_MARKER_PREFIX$token:"
        val endIndex = visibleOutput.indexOf(endMarkerPrefix)
        if (endIndex >= 0) {
            return visibleOutput.substring(0, endIndex)
        }

        return trimTrailingPartialMarker(visibleOutput, endMarkerPrefix)
    }

    private fun trimTrailingPartialMarker(
        text: String,
        marker: String
    ): String {
        val maxCandidateLength = minOf(text.length, marker.length - 1)
        for (candidateLength in maxCandidateLength downTo 1) {
            if (text.endsWith(marker.substring(0, candidateLength))) {
                return text.dropLast(candidateLength)
            }
        }
        return text
    }

    private suspend fun emitOutputChunkSafely(
        onOutputChunk: suspend (String) -> Unit,
        delta: String
    ) {
        if (delta.isEmpty()) {
        ...
        try {
            ...
            Log.w(TAG, "Hidden exec output callback failed", error)
        }
    }
    ...
    private suspend fun destroyHiddenExecShell(shell: HiddenExecShell) {
        withContext(Dispatchers.IO) {
    ...
    private fun buildHiddenExecStartupCommand(): Array<String> {
        val bash = resolveHostBashPath()
        val startScript =
            "source \$HOME/common.sh && install_ubuntu && configure_sources && fix_permissions && login_ubuntu '/bin/bash --noprofile --norc'"
        return arrayOf(bash, "-c", startScript)
    }

    private fun resolveHostBashPath(): String {
        val copiedBash = File(binDir, "bash")
        if (copiedBash.exists() && copiedBash.canExecute()) {
            return copiedBash.absolutePath
        }

        val nativeBash = File(nativeLibDir, "libbash.so")
    ...
    private fun buildEnvironment(): Map<String, String> {
        ...
        env["PATH"] = "${binDir.absolutePath}:${nativeLibDir}:${System.getenv("PATH")}"

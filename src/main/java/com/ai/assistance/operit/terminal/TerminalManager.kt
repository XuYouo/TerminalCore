...
class TerminalManager private constructor(
    ...
    suspend fun initializeEnvironment(
        ...
        if (isEnvInitialized) {
            return withContext(Dispatchers.IO) {
                try {
                    emitInitializationProgress(onProgress, "正在校验终端运行目录")
                    createDirectories()
                    emitInitializationProgress(onProgress, "正在补齐宿主二进制")
                    linkNativeLibs()
                    createBusyboxSymlinks()
                    emitInitializationProgress(onProgress, "正在校验 Ubuntu 运行资源")
                    extractAssets()
                    emitInitializationProgress(onProgress, "正在刷新启动脚本")
                    val startScript = generateStartScript()
                    File(filesDir, "common.sh").writeText(startScript.replace("\r\n", "\n").replace("\r", "\n"))
                    emitInitializationProgress(onProgress, "宿主终端环境校验完成")
                    true
                } catch (e: Exception) {
        ...
        try {
            ...
            val success = withContext(Dispatchers.IO) {
                try {
                    ...
                    emitInitializationProgress(onProgress, "正在创建终端运行目录")
                    createDirectories()

                    // 2. Link native libraries
                    emitInitializationProgress(onProgress, "正在准备 busybox/proot/bash")
                    linkNativeLibs()
                    createBusyboxSymlinks()

                    // 3. Extract assets
                    emitInitializationProgress(onProgress, "正在解压 Ubuntu 运行资源")
                    extractAssets()

                    // 4. Generate and write startup script
                    emitInitializationProgress(onProgress, "正在生成启动脚本")
                    val startScript = generateStartScript()
                    File(filesDir, "common.sh").writeText(startScript.replace("\r\n", "\n").replace("\r", "\n"))


                    Log.d(TAG, "Environment initialization completed successfully.")
                    emitInitializationProgress(onProgress, "终端运行时初始化完成")
                    true
                } catch (e: Exception) {
    ...
    private fun linkNativeLibs() {
        ...
        installNativeBinary(
            source = busyboxSo,
            destination = File(binDir, "busybox"),
            label = "busybox",
            executable = true,
            required = true
        )
        ...
        libraries.forEach { (libName, linkName) ->
            ...
            installNativeBinary(
                source = libFile,
                destination = File(binDir, linkName),
                label = linkName,
                executable = linkName != "libtalloc.so.2",
                required = linkName == "proot" || linkName == "loader" || linkName == "bash"
            )
        }
    }
    ...
    private fun installNativeBinary(
        ...
        if (!source.exists()) {
            Log.w(
                TAG,
                "Native library missing from nativeLibraryDir for $label: ${source.absolutePath}, trying APK extraction"
            )
            val extracted = extractNativeLibraryFromPackage(source.name, destination, executable)
            if (!extracted) {
                val message = "Native library not found for $label: ${source.absolutePath}"
                if (required) {
            ...
            Log.d(TAG, "Prepared $label at ${destination.absolutePath} (extracted-from-apk)")
            return
        }
        ...
        runCatching {
            ...
            Log.w(TAG, "Failed to delete existing $label at ${destination.absolutePath}", error)
        }
        ...
        val linked =
            runCatching {
                ...
                Log.w(TAG, "Symlink creation failed for $label, falling back to copy", error)
            }.isSuccess
        ...
        val valid = destination.exists() && (!executable || destination.canExecute())
        if (!valid) {
            val message = "Failed to materialize $label at ${destination.absolutePath}"
            if (required) {
                throw IOException(message)
            }
            Log.w(TAG, message)
        ...
        Log.d(
            TAG,
            "Prepared $label at ${destination.absolutePath} (${if (linked) "symlink" else "copy"})"
        )
    }

    private fun extractNativeLibraryFromPackage(
        libraryName: String,
        destination: File,
        executable: Boolean
    ): Boolean {
        ...
        val abiCandidates = Build.SUPPORTED_ABIS.toList() + listOf("arm64-v8a")

        for (apkPath in apkPaths) {
            val extracted =
                runCatching {
                    ZipFile(apkPath).use { zip ->
                        for (abi in abiCandidates.distinct()) {
                            ...
                            Log.d(
                                TAG,
                                "Extracted $libraryName from $apkPath!$entryName to ${destination.absolutePath}"
                            )
                            return true
                        }
                    }
                    ...
                    Log.w(TAG, "Failed to inspect APK for $libraryName: $apkPath", error)
                }.getOrDefault(false)
        ...
        install_ubuntu(){
          ...
          if [ -f "${'$'}OK_FILE" ]; then
            ...
            if [ -f "${'$'}UBUNTU_PATH/etc/issue.net" ]; then
              ...
              "${'$'}BIN/busybox" tar xf "${'$'}HOME/${'$'}UBUNTU" -C "${'$'}TMP_DIR"/ >/dev/null 2>&1
              if [ ${'$'}? -ne 0 ]; then
                cleanup_install
                trap - EXIT INT TERM
                return 1
...
$prootBindSetup
    ...
    private suspend fun emitInitializationProgress(
        onProgress: suspend (String) -> Unit,
        message: String
    ) {
        try {
            ...
            Log.w(TAG, "Environment progress callback failed", error)
        }
    }
